...
from . import baidu, google, youdao, papago
import traceback, re
import googletrans
...
async def dispatch(translator: str, src_lang: str, tgt_lang: str, texts: List[str], *args, **kwargs) -> List[str] :
	try:
		if translator not in ['google', 'youdao', 'baidu', 'papago', 'null'] :
			raise Exception
		if translator == 'null' :
			return texts
		...
		for old, new in postTL:
				result = [re.sub(old, new, x) for x in result]
...
async def test() :
	src = '测试'
	print(await dispatch('papago', 'auto', 'ENG', [src]))
